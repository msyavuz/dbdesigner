...
export type ProjectsResponseType = Awaited<ReturnType<typeof client.projects.$get>>
...
export const fetchProject = async (projectId: string) => {
  ...
  if (!response.ok) {
    return Promise.reject(
      new Error(`Failed to fetch project with id ${projectId}: ${response.statusText}`)
    )
  }
  return response.json()
}
...
export const createProject = async (data: {
  ...
  if (!response.ok) {
    return Promise.reject(new Error(`Failed to create project: ${response.statusText}`))
  }
  return response.json()
}
...
export const deleteProject = async (projectId: string) => {
  ...
  if (!response.ok) {
    return Promise.reject(
      new Error(`Failed to delete project with id ${projectId}: ${response.statusText}`)
    )
  }
  return response.json()
}
...
export const updateProject = async (
  ...
  if (!response.ok) {
    return Promise.reject(
      new Error(`Failed to update project with id ${projectId}: ${response.statusText}`)
    )
  }
  return response.json()
}
...
export const sendAIMessage = async function* (projectId: string, message: string) {
